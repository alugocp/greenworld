...
class IwebXlsDataCollector(BaseDataCollector):
    ...
    def collect_data(self, key: str) -> dict:
        ...
        if cli_options["row-headers"] is not None and isinstance(
            cli_options["row-headers"][0], int
        ):
            cli_options["row-headers"] = extract_range_from_file(
                csv_filename, cli_options["row-headers"]
            )
            cli_options["row-headers"] = list(
                map(parse_species_name, cli_options["row-headers"])
            )
        if cli_options["col-headers"] is not None and isinstance(
            cli_options["col-headers"][0], int
        ):
            cli_options["col-headers"] = extract_range_from_file(
                csv_filename, cli_options["col-headers"]
            )
            cli_options["col-headers"] = list(
                map(parse_species_name, transposed(cli_options["col-headers"]))
            )
        ...
        if len(data[0]) != len(cli_options["col-headers"]):
            raise ValueError(
                "Mismatched column headers length with number of data columns"
            )
        ...
        output = {
            "plants": [],
            "others": [],
            "works_cited": [{"id": 1, "citation": cli_options["citation"]}],
        }
