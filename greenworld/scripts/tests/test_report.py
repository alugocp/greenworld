    ...
    with db.connect() as con:
        data = con.execute(
            reports_table.select().where(
                sqlalchemy.and_(
                    reports_table.c["plant1"] == id1, reports_table.c["plant2"] == id2
                )
            )
        ).mappings()
...
class ScriptsReportCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with db.connect() as con:
            con.execute(
                works_cited_table.insert().values(
                    [{"id": 1, "citation": "Trust me, bro"}]
                )
            )
            con.execute(
                other_species_table.insert().values(
                    [
                        {"id": 1, "species": "fabaceae", "name": "Bean Family"},
                        {"id": 2, "species": "bee species", "name": "Bee"},
                        {"id": 3, "species": "butterfly species", "name": "Butterfly"},
                        {"id": 4, "species": "fungus species", "name": "Mushroom"},
                    ]
                )
            )
            con.execute(
                plants_table.insert().values(
                    [
                        {
                            "id": 1,
                            "name": "Bean",
            ...
            con.execute(
                ecology_other_table.insert().values(
                    [
                        {
                            "plant": 3,
                            "non_plant": 1,
            ...
            con.execute(
                ecology_plant_table.insert().values(
                    [
                        {
                            "plant": 4,
                            "target": 3,
    ...
    def test_nitrogen(self):
        ...
        self.assertEqual(
            get_report(2, 2), ["Pumpkin and Pumpkin are both heavy feeders"]
        )

        self.assertEqual(get_report(2, 3), [])
